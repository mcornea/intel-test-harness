...
class CfgApi:
    cfg_file_name: str = field(init=False, default=None)
    wls_mem_size_str: str = "wlsMacMemorySize"
    wls_mem_size_val: int = 0x9EA80000
...
class CfgData:
    ...
    @classmethod
    def read_api_yaml(cls, yaml_data: Any):
        api_data = yaml_data["Api_cfgs"]

        for num in api_data:
            cfg_objs = api_data[num]
            for cfg in cfg_objs:
                api_objs = cfg_objs[cfg]
                cfg_file = cfg + ".xml"
                for api_field in api_objs:
                    cls.load_api_cfg(cfg_file, api_field, api_objs[api_field])

    @classmethod
    def load_api_cfg(cls, cfg_file: str, cfg_field: str, cfg_val: Any):
        a_api_cfg = None
        if cfg_file not in cls.dict_cfg_api.keys():
            a_api_cfg = CfgApi()
            a_api_cfg.cfg_file_name = cfg_file
        else:
            a_api_cfg = cls.dict_cfg_api[cfg_file]

        if a_api_cfg.wls_mem_size_str == cfg_field:
            a_api_cfg.wls_mem_size_val = cfg_val

        if cfg_file not in cls.dict_cfg_api.keys():
            cls.dict_cfg_api[cfg_file] = a_api_cfg

    @classmethod
    def update_api_cfg_xml(cls, cfg_file_name: str, api_cfg: CfgApi):
        assert(cfg_file_name in cls.dict_cfgfile_paths.keys())
        try:
            with open(cls.dict_cfgfile_paths[cfg_file_name] + cfg_file_name, encoding="utf8") as f:
                ...
                root_api_cfg = root.find('Api')
                xml_api_mem_size = root_api_cfg.find(api_cfg.wls_mem_size_str)
                if xml_api_mem_size == None:
                    print("Cound not find the existing wls memory size config")
                else: 
                    xml_api_mem_size.text = hex(api_cfg.wls_mem_size_val)
                xml_tree.write(cls.dict_cfgfile_paths[cfg_file_name] + cfg_file_name)
        except Exception as e:
            sys.exit("Could not handle api xml: %s" %e)

    @classmethod
    def process_update_api_cfg_xml(cls):
        for cfg_file in cls.dict_cfg_api.keys():
            cls.update_api_cfg_xml(cfg_file, cls.dict_cfg_api[cfg_file]);
    ...
    def read_cfg_yaml(cls, cfg_yaml, cpu_resource: CpuResource):
        ...
        try:
            with open(cfg_yaml) as fsrc:
                ...
                if "Api_cfgs" in yaml_data.keys():
                    cls.read_api_yaml(yaml_data)
    ...
    def process_cfg_xml(cls, yaml_file_name, cpu_resource: CpuResource):
        ...
        cls.process_update_api_cfg_xml()
